...
def process_label(intents, w2v,ctxEmb='emb',class_id_startpoint=0):
    ...
    if ctxEmb=='bert':
        ...
        word_embedding = BertModel.from_pretrained(bert_name)
    for line in intents:
        ...
        if ctxEmb == 'bert':
            x_tokens = w
            x_tokens = tokenizer(x_tokens, return_tensors='pt')
            x_arr = word_embedding(**x_tokens)
            label_sum = x_arr.last_hidden_state[0][0].detach().numpy()
        else:
...
def load_vec(file_path, w2v, in_max_len, unSeen=True,ctxEmb='emb'):
    """ load input data
        input:
    ...
    for line in open(file_path,'rb'):
        ...
        x_arr = []
        if ctxEmb == 'bert':
            x_tokens = "[CLS] " + arr[1] + "[SEP]"
            x_tokens = tokenizer.tokenize(x_tokens)
            x_arr = tokenizer.convert_tokens_to_ids(x_tokens)
            # x_tokens = arr[1]
            # x_tokens = tokenizer(x_tokens, return_tensors='pt')
        else:
            for w in question:
...
def read_datasets(dataSetting):
    ...
    # bert_name = dataSetting['bertname']
    ctxEmb = dataSetting['ctxEmb']
    
    max_len = 0
    if dataSetting['test_mode']==1:
        x, y, class_set, s_lens, max_len, ind_del= load_vec(data_path, w2v, max_len,False,ctxEmb)
    else:
        x, y, class_set, s_lens, max_len, ind_del= load_vec(data_path, w2v, max_len,True,ctxEmb)
...
# split training set and test set
    ...
    ind_tr = []        
    if dataSetting['test_mode']==1:
        matlabData=sio.loadmat(dataSetting['data_prefix']+dataSetting['sim_name_withOS'])
        ind_te.extend((matlabData['test_ind'][0,dataSetting['id_split']]-1).tolist()[0])
        ind_tr.extend((matlabData['train_ind'][0,dataSetting['id_split']]-1).tolist()[0])
        data['id_split']=dataSetting['id_split']
        data['sim']=matlabData['similarity'][0,dataSetting['id_split']]
        for i in ind_del:
...
#            ind_tr.extend(ind_tr_temp)
        ...
        matlabData=sio.loadmat(dataSetting['data_prefix']+dataSetting['sim_name_withS'])
        data['sim']=matlabData['similarity']
#        # sim for M zscore
#        matlabData=sio.loadmat(dataSetting['data_prefix']+dataSetting['sim_name_withS'])
#        simtemp=np.array(matlabData['similarity'])
#        ind_us=[indx for indx, x in enumerate(labelorder) if x in unseen_class]
#        ind_sc=[indx for indx, x in enumerate(labelorder) if x in seen_class]
#        data['sim']=simtemp[ind_us,][:,ind_sc]
        for i in range(len(seen_class)):
    ...
    sc_dict, sc_vec = process_label(seen_class, w2v,ctxEmb,class_id_startpoint)
    
    if dataSetting['test_mode']==1:
        uc_dict, uc_vec = process_label(unseen_class, w2v,ctxEmb,class_id_startpoint+len(sc_dict))
        uc_dict=dict(sc_dict,**uc_dict)
        uc_vec = np.concatenate([sc_vec,uc_vec],axis=0)  
    else:
        uc_dict, uc_vec = process_label(unseen_class, w2v,ctxEmb,class_id_startpoint)
